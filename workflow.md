...
- **README --ocr docs** — PENDING. `docs/cli-reference/ocr-flag.md` created by Build Implementer. README flags table and OCR section still need a pass: add `--ocr` row to flags table and add an OCR section (matching the `--json` treatment in the README). Next Docs Writer session must pick this up before Sprint 2 gate is closed.
